...
@retry(wait=wait_random_exponential(min=10, max=30), stop=stop_after_attempt(5))
def fetch_Gemini_lite_response(instruction, system_prompt, temperature=0.0):
    model = genai.GenerativeModel(
...
@memory.cache
def get_Gemini_lite_response(instruction, system_prompt, temperature=0.0):
    res = fetch_Gemini_lite_response(instruction, system_prompt, temperature)
    return res
