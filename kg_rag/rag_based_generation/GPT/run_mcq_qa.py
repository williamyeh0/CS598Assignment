...
JSON_SYSTEM_PROMPT = system_prompts["DISEASE_GENE_VARIANT_EXTRACTION"]
...
# read in prior knowledge file
prior_knowledge=None
prior_knowledge_path = "knowledge.txt"
...
def main():
    ...
    # slice for start index
    # start_index = 200
    # end_index = 200
    # subset_df = question_df.iloc[start_index:]   # Slices rows 271 → end
    ...
    for index, row in tqdm(question_df.iterrows(), total=306):
        try: 
            ...
            if MODE == "1":
                ### MODE 1: jsonlize the context from KG search ### 
                """
                LLM based jsonlization using flash lite 
                """
                context = retrieve_context(row["text"], vectorstore, embedding_function_for_context_retrieval, node_context_df, CONTEXT_VOLUME, QUESTION_VS_CONTEXT_SIMILARITY_PERCENTILE_THRESHOLD, QUESTION_VS_CONTEXT_MINIMUM_SIMILARITY, edge_evidence, model_id=CHAT_MODEL_ID)
                # print(f"context: {context}")
                json_context = get_Gemini_lite_response(context, JSON_SYSTEM_PROMPT, temperature=TEMPERATURE)
            ...
            if MODE == "2":
                ...
                context = retrieve_context(row["text"], vectorstore, embedding_function_for_context_retrieval, node_context_df, CONTEXT_VOLUME, QUESTION_VS_CONTEXT_SIMILARITY_PERCENTILE_THRESHOLD, QUESTION_VS_CONTEXT_MINIMUM_SIMILARITY, edge_evidence, model_id=CHAT_MODEL_ID)
                enriched_prompt = "Context: "+ context + "\n" + "PRIOR KNOWLEDGE AND NOTES: " + prior_knowledge + "\n" + "Question: "+ question
                output = get_Gemini_response(enriched_prompt, SYSTEM_PROMPT, temperature=TEMPERATURE)
